...
description: >-
  Use when the user provides Figma URLs or node IDs and asks to implement the
  design in production code with 1:1 visual fidelity. Requires a working
  Figma MCP server; context fetching or MCP troubleshooting alone is figma.
---
...
Ensure the Figma MCP server is connected. Setup depends on your harness — e.g. `claude mcp add` for Claude Code, or your harness's MCP config file — point it at `https://mcp.figma.com/mcp` and complete the OAuth login. Restart the harness after connecting. Then you can continue with Step 1.
