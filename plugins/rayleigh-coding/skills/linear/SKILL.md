...
name: linear
description: >-
  Use when the user asks to read, create, or update issues, tickets, or
  projects in Linear, or mentions Linear team workflows. Requires the Linear
...
Ensure the Linear MCP server is connected. Setup depends on your harness — e.g. `claude mcp add` for Claude Code, or your harness's MCP config file — point it at `https://mcp.linear.app/mcp` and complete the OAuth login. Restart the harness after connecting. Then you can continue with Step 1.
