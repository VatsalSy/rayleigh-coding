---
name: "sentry"
description: "Use when the user asks to inspect Sentry issues or events, \"summarize recent production errors\", or pull Sentry health data — read-only API queries via the bundled script; requires `SENTRY_AUTH_TOKEN`."
---
...
```bash
export SKILL_DIR="/path/to/sentry"
export SENTRY_API="$SKILL_DIR/scripts/sentry_api.py"
```

`$SKILL_DIR` refers to the directory containing this skill. The invoking agent resolves this at runtime.
