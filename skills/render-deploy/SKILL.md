---
name: render-deploy
description: Use when the user asks to deploy, host, publish, or set up an application on Render — analyses the codebase, generates a render.yaml Blueprint, and provides Dashboard deeplinks.
---
...
3) Add the MCP server via your harness's MCP config — point it at `https://mcp.render.com/mcp` with `RENDER_API_KEY` as the bearer token env var.

4) Restart your harness, then retry `list_services()`.
...
3. **Review + merge**: create a focused branch, validate and review
   `render.yaml`, open a PR, and merge it before deployment.
...
Use a focused branch and pull request by default. Run the repository-aware
`code-review` guard once with `--uncommitted` (see `CONVENTIONS.md` "Code
review before push and merge"), but only after proving every tracked staged and
unstaged edit belongs to the intended `render.yaml` change and the diff contains
no credentials. Stop and resolve any scope mismatch before transmitting it.
Fix verified Critical/Warning findings. If
the CLI or capacity is unavailable, record that and rely on the repository
checks; do not retry an unchanged diff. Commit, push, open the PR through
`gh-pr-create`, satisfy its merge gate, and merge before opening the Blueprint
deeplink. Only bypass the PR when the user explicitly authorises a direct push.
...
If deployment fails due to network issues (timeouts, DNS errors, connection resets), rerun the deploy with escalated permissions (use the agent's escalation mechanism). The deploy requires escalated network access when sandbox networking blocks outbound requests.

Example guidance to the user:

```
The deploy needs escalated network access to deploy to Render. I can rerun with escalated permissions—want me to proceed?
```
