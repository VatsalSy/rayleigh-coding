---
name: "screenshot"
description: "Use when the user says \"take a screenshot\", \"capture this window\", or asks for a desktop/system capture — full screen, a specific app or window, or a pixel region — or when an OS-level capture must replace unavailable tool-specific capture."
---
...
> [!important] Cowork vs Local
> - **Cowork (sandboxed):** Python helpers, `screencapture`, and `scrot` are NOT available. Use `mcp__computer-use__screenshot` directly (and `mcp__computer-use__zoom` for detail). See §Cowork section below.
> - **Local terminal (Jarvis / controller-b / plain shell):** Use the Python helper scripts and OS commands as described below.

## Cowork (computer-use)

In Cowork, the `mcp__computer-use__*` tools replace all local screenshot tooling.
...
**Zoomed region (for reading small text or detail):**
```
mcp__computer-use__zoom(x, y, width, height)
```

**Capture a specific app:**
1. `mcp__computer-use__request_access(["<AppName>"])`
2. `mcp__computer-use__open_application("<AppName>")`
3. `mcp__computer-use__screenshot()`

Screenshots taken in Cowork are returned inline — they do not save to disk unless you write the image data explicitly. For agent inspection, just view the returned screenshot directly.

---

## Local section (controller-b / macOS / Linux)
...
The helpers route Swift's module cache to `$TMPDIR/swift-module-cache`
...
```bash
bash <path-to-skill>/scripts/ensure_macos_permissions.sh && \
python3 <path-to-skill>/scripts/take_screenshot.py --app "<APP_NAME>"
```
...
```bash
bash <path-to-skill>/scripts/ensure_macos_permissions.sh && \
python3 <path-to-skill>/scripts/take_screenshot.py --app "<APP_NAME>" --mode temp
```
...
```bash
python3 <path-to-skill>/scripts/take_screenshot.py --app "<APP_NAME>" --window-name "Settings"
```

- List matching window ids before capturing (macOS only):

```bash
python3 <path-to-skill>/scripts/take_screenshot.py --list-windows --app "<APP_NAME>"
```
