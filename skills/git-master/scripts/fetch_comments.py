...
Requires:
  - `gh auth status` (via bot wrapper) already set up
...
def _ensure_gh_authenticated() -> None:
    try:
        _run([GH_CMD, "auth", "status"])
    except RuntimeError:
        print("run `gh auth status` to authenticate the GitHub CLI", file=sys.stderr)
        raise RuntimeError("gh auth status failed; run `gh auth status` to authenticate the GitHub CLI") from None


def gh_pr_view_json(fields: str) -> dict[str, Any]:
    # fields is a comma-separated list like: "number,headRepositoryOwner,headRepository"
    return _run_json([GH_CMD, "pr", "view", "--json", fields])
