...
description: >-
  Use when the user asks "who owns this sensitive code", "bus factor", "orphaned sensitive code", "CODEOWNERS reality check", or asks about security ownership clusters.
---
...
- **Large repos with many merge commits will distort bus-factor counts.** Dependabot, CI bot, and release-commit authors inflate touch counts for common files. Always use `--no-default-author-excludes` as a verification step and add a `--author-exclude-regex` for known bots before reading bus-factor results as meaningful.
- **`networkx` community detection is non-deterministic across runs.** Community IDs in `communities.json` are not stable between runs on the same repo if the graph changes. Do not hardcode community IDs in downstream queries; always look up by file or person.
- **Co-change edges on monorepos have supernode files that corrupt clustering.** Files like `package-lock.json`, `Cargo.lock`, `*.github/*` appear in almost every commit and pull unrelated files into the same community. The default excludes help but may not be enough for all repos. Review `cochange_edges.csv` for implausibly high Jaccard weights on infra files before presenting clusters.
...
```bash
python <skills-dir>/security-ownership-map/scripts/run_ownership_map.py \
  --repo /path/to/linux \
...
Run from the repo root (skill lives at `<skills-dir>/security-ownership-map/`):
...
```bash
python <skills-dir>/security-ownership-map/scripts/query_ownership.py --data-dir ownership-map-out people --limit 10
python <skills-dir>/security-ownership-map/scripts/query_ownership.py --data-dir ownership-map-out files --tag auth --bus-factor-max 1
python <skills-dir>/security-ownership-map/scripts/query_ownership.py --data-dir ownership-map-out person --person alice@corp --limit 10
python <skills-dir>/security-ownership-map/scripts/query_ownership.py --data-dir ownership-map-out file --file crypto/tls
python <skills-dir>/security-ownership-map/scripts/query_ownership.py --data-dir ownership-map-out cochange --file crypto/tls --limit 10
python <skills-dir>/security-ownership-map/scripts/query_ownership.py --data-dir ownership-map-out summary --section orphaned_sensitive_code
python <skills-dir>/security-ownership-map/scripts/query_ownership.py --data-dir ownership-map-out community --id 3
```
...
python <skills-dir>/security-ownership-map/scripts/query_ownership.py --data-dir ownership-map-out summary --section hidden_owners

# Sensitive hotspots with low bus factor
python <skills-dir>/security-ownership-map/scripts/query_ownership.py --data-dir ownership-map-out summary --section bus_factor_hotspots

# Auth/crypto files with bus factor <= 1
python <skills-dir>/security-ownership-map/scripts/query_ownership.py --data-dir ownership-map-out files --tag auth --bus-factor-max 1
python <skills-dir>/security-ownership-map/scripts/query_ownership.py --data-dir ownership-map-out files --tag crypto --bus-factor-max 1

# Who is touching sensitive code the most
python <skills-dir>/security-ownership-map/scripts/query_ownership.py --data-dir ownership-map-out people --sort sensitive_touches --limit 10

# Co-change neighbors (cluster hints for ownership drift)
python <skills-dir>/security-ownership-map/scripts/query_ownership.py --data-dir ownership-map-out cochange --file path/to/file --min-jaccard 0.05 --limit 20

# Community maintainers (for a cluster)
python <skills-dir>/security-ownership-map/scripts/query_ownership.py --data-dir ownership-map-out community --id 3

# Monthly maintainers for the community containing a file
python <skills-dir>/security-ownership-map/scripts/community_maintainers.py \
