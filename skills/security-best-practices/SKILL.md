...
description: "Use when the user asks for a security review of code, and whenever shell scripts or HTML/JS string-building logic change (argument handling, DOM sanitisation). Language-specific best-practice checks for Python, JavaScript/TypeScript, Go, and shell command-execution paths."
---
...
High-signal triggers from prior incidents:
- Shell script edits (`*.sh`, deploy scripts, command wrappers): prioritize command-injection prevention and robust argument handling.
- HTML/JS string construction edits (template literals, `innerHTML`, dynamic markup): prioritize DOM injection/XSS prevention and safe sanitization patterns.
...
Before deep review, run a rapid risk scan for these two classes:
1. Shell command execution paths:
   - Unquoted variables in command contexts
   - `eval`/`bash -c` with interpolated data
   - Missing allowlists/validation for user-provided arguments
   - Missing `--` or array-safe argument passing when relevant
2. HTML/JS rendering paths:
   - Unsanitized data inserted into `innerHTML`/HTML strings
   - Event-handler string injection (`onclick="..."` style patterns)
   - URL/attribute injection in dynamic anchors/images/scripts
   - Missing sanitizer or safer alternatives (`textContent`, DOM APIs, vetted sanitizer)

As a corroborating pass, run the repository-aware `code-review` guard on the
changed code with `--uncommitted` or `--base main`, and reconcile its security
findings with the manual scan. It catches different classes than the checklist.
Caveat: the CLI sends diffs to the CodeRabbit API; when the code
under review handles secrets or credentials, confirm none are present in the
diff itself before running it, and skip the pass entirely if they are (see
`code-review/SKILL.md` Security).
...
**Bad:** "This script has injection issues; sanitise inputs." **Good:** "`deploy.sh` line 42 interpolates `$BRANCH` unquoted inside `eval` — a branch named `x; rm -rf ~` executes; quote the variable and drop the `eval`."
...
- **Shell scripts with unquoted variables are the highest-frequency real vulnerability here.** In CoMPhy scripts, `runSimulation.sh` and sweep runners routinely do `cd $DIR` or `rm -rf $OUTPUT` without quoting. A path with a space in the name causes silent word-splitting. Always quote: `"$DIR"`, `"$OUTPUT"`.
- **Security reports that reference wrong line numbers are worse than no report.** Line numbers shift after every edit. Verify line numbers immediately before writing the report, not against a cached read. If referencing a finding in a script, quote the actual code snippet alongside the line number.
- **TLS warnings should not be filed as findings for local/dev Basilisk scripts.** These are command-line CFD runners, not web services. Flagging "no HTTPS" on a local simulation script wastes the user's review time. Scope findings to actual exposure paths.

## Review Prioritization Principles (Source: clawhub:ivangdavila/security-best-practices)

**No evidence, no finding.** Before drawing conclusions, confirm:
...
**Prioritize exploitability over theory.** Rank findings by practical risk, not checklist volume:
- Reachability from untrusted inputs
- Privilege required by attacker
- Blast radius if exploited
- Ease of abuse and repeatability

A speculative XSS with no user-controlled input path is lower priority than an unquoted variable in a deploy script that runs as root.
