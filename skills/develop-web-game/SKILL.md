...
description: >-
  Use when building or iterating on a web game (HTML/JS) — the user says "make
  the game", "fix the gameplay", or asks to test game behaviour with a
  Playwright loop, screenshots, or render_game_to_text console review.
---
...
The invoking agent resolves the skill directory at runtime. Configure these variables based on your agent platform:

```bash
export WEB_GAME_CLIENT="<SKILL_ROOT>/scripts/web_game_playwright_client.js"
export WEB_GAME_ACTIONS="<SKILL_ROOT>/references/action_payloads.json"
```

Replace `<SKILL_ROOT>` with the path to this skill's directory on your system.
